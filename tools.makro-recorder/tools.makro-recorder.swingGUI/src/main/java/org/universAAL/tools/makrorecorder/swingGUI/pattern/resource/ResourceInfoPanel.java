...
public class ResourceInfoPanel extends JPanel {
	...
	public static String genInfo(ServiceRequest sr) {
		// return
		// "<h2>Service-Request</h2><pre>"+Activator.getSerializer().serialize(sr)+"</pre>";
		String info = "";
		...
		if (effects.length > 0) {
			...
			for (Resource effect : effects) {
				for (String type : effect.getTypes()) {
					info += "<li>" + shortURI(type) + " "
							+ effect.getProperty("http://ontology.universAAL.org/Service.owl#propertyValue").toString()
							+ "</li>";
				}
			}
		}
