...
public class Sparul {
	...
	private void getOutputs(StringBuilder s, StringBuilder where, ServiceRequest sr) {
		...
		for (int i = 0; i < outputs.length; i++) {
			...
			if (output == null)
				continue;
			Resource bind = (Resource) output.getProperty(OutputBinding.PROP_OWLS_BINDING_TO_PARAM);
			PropertyPath pp = (PropertyPath) output.getProperty(OutputBinding.PROP_OWLS_BINDING_VALUE_FORM);
	...
	private void getEffects(StringBuilder s, StringBuilder where, ServiceRequest sr) {
		...
		for (int i = 0; i < effects.length; i++) {
			...
			if (effect == null)
				continue;
			PropertyPath pp = (PropertyPath) effect.getProperty(ProcessEffect.PROP_PROCESS_AFFECTED_PROPERTY);
			if (pp == null)
				continue;
			...
			Object value = effect.getProperty(ProcessEffect.PROP_PROCESS_PROPERTY_VALUE);
			if (value == null)
				if (type != EFF_REMOVE)
					continue;
