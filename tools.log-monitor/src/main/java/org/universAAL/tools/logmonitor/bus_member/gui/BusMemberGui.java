	...
	public void add(final MemberData m) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				// handle module
				DefaultMutableTreeNode moduleNode = modules.get(m.peer + "#" + m.module);
				if (moduleNode == null) {
				...
				if (memberNode == null) {
					...
					// case?)
					memberNode = new DefaultMutableTreeNode(
							"Member: " + m.busNameReadable + "-" + m.type + " - " + m.number);
					moduleNode.add(memberNode);
					// treeModel.reload(moduleNode);
					treeModel.nodesWereInserted(moduleNode, new int[] { moduleNode.getChildCount() - 1 });
	...
	public void remove(final String busMemberID) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (m == null) {
					System.out.println("ERROR: member data not found while removing");
					return;
				}
				...
				if (memberNode == null) {
					System.out.println("ERROR: member node not found while removing");
					return;
				}

				final MutableTreeNode moduleNode = (MutableTreeNode) memberNode.getParent();
				...
				if (moduleNode.getChildCount() == 0) {
					...
					if (modules.remove(m.peer + "#" + m.module) == null)
						System.out.println("ERROR: module does not exist while removing");
				}
			}
		});
	}
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				if (members.containsValue(node)) {
					...
					String peerID = node.getUserObject().toString().substring(PREFIX_PEER.length());
