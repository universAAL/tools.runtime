...
public class RDFVis extends JPanel implements ListSelectionListener {
	...
	public RDFVis() {
		...
		// Split pane
		JSplitPane split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, scrollpaneTable, scrollpane);
	...
	static void saveAll() {
		if (instance != null) {
			...
			// instance.dir = ""+myDir+ "\\";
			String myDir = "";
			String textFilename = myDir + "LogMonitor_" + instance.messages.get(0).getDateString().replace(':', '-')
					+ ".txt";
			...
			synchronized (instance.messages) {
				...
				for (Message m : instance.messages) {
					...
					Point p = instance.panel.drawNodes(g2, m.node, 20, 20, new Point(0, 0));
					p.y += 20;
					...
					instance.panel.drawNodes(g2, m.node, 20, 20, new Point(0, 0));

					// save
					String filename = myDir + m.getDateString().replace(':', '-') + "_";
					...
					System.out.println("File: width " + p.x + " height " + p.y + " " + outFile.getAbsolutePath()
							+ outFile.getName());
					try {
					...
					if (writer != null) {
						try {
							...
							writer.write(m.cls + "->" + m.method + " (Interpretation: " + m.shortDescription + ")\n");
	 ...
	 */
	public void addMessage(String cls, String method, String message, Resource r, String shortDescription) {
	 ...
	 *            The date.
	 */
	private void addMessage(String cls, String method, String message, Node node, Date date, String shortDescription) {
		...
		synchronized (messages) {
			...
			model.addRow(new Object[] { m.getDateString(), cls, method, message, shortDescription });
