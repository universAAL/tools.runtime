...
public class CEStringUtil {
	...
	public static String toString(String indent, Resource r, boolean shortForm) {
		if (ProcessParameter.TYPE_OWLS_VALUE_OF.equals(r.getType())) {
			ProcessInput theVar = (ProcessInput) r.getProperty(ProcessParameter.PROP_OWLS_VALUE_OF_THE_VAR);
			...
			return indent + type + ":\n" + indent + "   Type " + URI.get(parameterType, shortForm) + "\n" + indent
					+ "   with cardinality (" + minCard + ", " + maxCard + ")\n";
		}

		return indent + "<unknown object: " + r.toStringRecursive(indent + "  ", false, null) + ">\n";
	}

	public static String toString(String indent, ManagedIndividual o, boolean shortForm) {
	...
	public static String toString(String indent, PropertyRestriction restr, boolean shortForm) {
		String s = indent + URI.get(restr.getClassURI(), true) + " on property "
				+ URI.get(restr.getOnProperty(), shortForm) + "\n";
		s += toString(indent + "  ", restr.getConstraint(), shortForm);
		return s;
	}

	public static String toString(String indent, MergedRestriction restr, boolean shortForm) {
		String s = indent + "MergedRestriction on property " + URI.get(restr.getOnProperty(), shortForm) + "\n";
		List<?> l = restr.getRestrictions();
		for (int i = 0; i < l.size(); i++)
			s += toString(indent + "  ", (PropertyRestriction) l.get(i), shortForm);
		return s;
	}
}
