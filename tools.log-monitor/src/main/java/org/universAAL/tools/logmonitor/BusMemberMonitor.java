...
public class BusMemberMonitor implements DistributedBusMemberListener {
	...
	public void start() {
		...
		DistributedBusMemberManager registry = (DistributedBusMemberManager) Activator.mc.getContainer()
				.fetchSharedObject(Activator.mc, new Object[] { DistributedBusMemberManager.class.getName() });
	...
	public void busMemberAdded(PeerCard origin, String busMemberID, String busName, BusMemberType memberType,
		...
		MemberData data = new MemberData(origin, busMemberID, busName, memberType, label, comment);
		synchronized (listeners) {
