	...
	private void getOverviewHTML(StringBuilder s, List<SingleMatching> group) {
		for (Iterator<SingleMatching> it = group.iterator(); it.hasNext();) {
			...
			if (single.success.booleanValue())
				...
				s.append(getTableRowHTML(getImageHTML("ERROR_16.png"), val2, val3));
		}
	}
	...
	private String createHTML(Matchmaking m) {
		...
		if (m.registeredServicesAvailable) {
			...
			if (overviewMethod == 0) {
				...
				ArrayList<String> sortedProviderURIs = new ArrayList<String>(groups.keySet());
				...
				for (String providerURI : sortedProviderURIs) {
					...
					s.append(getTableRowHTML("Provider: " + getURIHTML(providerURI), 3));
		...
		// details for request
		s.append("Request: ");
		getAllServiceRequestHTML(s, m.request, m.serviceURI, m.serializedRequest);
		...
		if (m.registeredServicesAvailable) {
			...
			for (Iterator<SingleMatching> it = l.iterator(); it.hasNext();) {
				SingleMatching single = it.next();
				s.append("<hr><h2>Details for " + URI.get(single.serviceURI, true) + "</h2>\n\n");
					...
					case SingleMatching.REASON_INPUT:
						if (!single.isOffer) {
							...
							if (prop == null)
								...
								s.append("   input: input parameters do not match for property " + getURIHTML(prop));
							s.append("<br>\n");
						}
						s.append("   input: ");
			...
			s.append("<hr><h2>Provider filtering</h2><p>The list of services is filtered,"
					...
					+ " most specialized service is taken.</p>\n<p>The number of matching" + " services is:</p>\n");

			s.append(getTableStartHTML(0));
			s.append(getTableRowHTML("before provider filtering:", "" + m.numMatchingProfiles));
			s.append(getTableRowHTML("after provider filtering:", "" + m.numMatchingAfterProviderFilter));
			s.append(getTableEndHTML());
			...
			for (SingleMatching single : l) {
				if (single.success.booleanValue()) {
					...
					String res = m.matchingsProvFilt.contains(profileURI) ? "OK_16.png" : "ERROR_16.png";
					s.append(getTableRowHTML(getImageHTML(res), getURIHTML(profileURI)));
				}
			}
			s.append(getTableEndHTML());
