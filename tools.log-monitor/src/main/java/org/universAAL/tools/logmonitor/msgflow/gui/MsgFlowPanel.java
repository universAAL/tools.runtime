...
public class MsgFlowPanel extends JPanel {
	...
	public final static Color busColor[] = new Color[] { new Color(74, 172, 197), new Color(154, 186, 87),
				...
				synchronized (messages) {
					for (FlowData d : messages) {
						...
						if (dist < msgHeight && dist >= 0) {
							...
							LogMonitor.instance.gui.setSelection((Integer) d.ref);
							break;
						}
					}
				}
	...
	public class MyViewport extends JViewport {
		...
		public void paint(Graphics graphics) {
			...
			g.fillRect(0, 0, (int) getVisibleRect().getWidth(), titleHeight + 10);
			...
			synchronized (members) {
				...
				for (MemberDataEx dat : members) {
					...
					g.setStroke(thick);
					g.drawLine(x - memberDist / 3, titleHeight + 10, x + memberDist / 3, titleHeight + 10);
					g.setStroke(dashed);
	 ...
	 * Create the panel.
	 */
	public MsgFlowPanel(JScrollBar scrollBarHorizontal, JScrollBar scrollBarVertical) {
		instance = this;
	...
	private void startContextMenu(int x, int y) {
		...
		for (int busidx = 0; busidx < 3; busidx++) {
			String name = (busActive[busidx] ? "Hide" : "Show") + " all " + busNames[busidx] + " Bus members";
		...
		if (y2 < titleHeight) {
			// click on a member?
			int idx = (x - member1Pos + memberDist / 3 - (titleHeight - y2)) / memberDist;
			if (idx >= 0 && idx < numCurrentlyShownMembers) {
				...
				if (member != null) {
					...
					menu.add(new AbstractAction("Hide bus member \"" + member.name + "\"", null) {
						private static final long serialVersionUID = 1L;
	...
	protected void paintComponent(final Graphics graphics) {
		...
		synchronized (messages) {
			...
			for (FlowData d : messages) {
				...
				if (d.bus == 0) {
					...
					g.drawString(date2String(d.date), 5, y);
					g.setColor(busColor[d.bus]);
					g.drawLine(member1Pos - memberDist / 3, y, xname - memberDist * 2 / 3, y);
				}
				g.setColor(c);
		...
		Point p = new Point(member1Pos + memberDist * numCurrentlyShownMembers + 500, y);
