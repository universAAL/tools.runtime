...
public class HTMLBusOperationsPane extends HTMLVisibilityPane {
	 ...
	 * @param profileURI
	 */
	protected void getAllContextEventPatternHTML(StringBuilder s, PatternInfo info) {
	...
	private List<PropertyRestriction> getRestrictions(ContextEventPattern cep, String onProperty) {
	...
	protected void getContextEventPatternHTML(StringBuilder s, PatternInfo info) {
		...
		if (info.serializedSubject == null)
			info.serializedSubject = getCEPSerialization(cep, ContextEvent.PROP_RDF_SUBJECT);
		if (info.serializedPredicate == null)
			info.serializedPredicate = getCEPSerialization(cep, ContextEvent.PROP_RDF_PREDICATE);
		if (info.serializedObject == null)
			info.serializedObject = getCEPSerialization(cep, ContextEvent.PROP_RDF_OBJECT);

		s.append(getTableStartHTML());
		s.append(getVTableRowWithTitleHTML("subject", info.serializedSubject));
		s.append(getVTableRowWithTitleHTML("predicate", info.serializedPredicate));
	...
	protected String getServiceOutputHTML(Resource output) {
		...
		Object form = output.getProperty(OutputBinding.PROP_OWLS_BINDING_VALUE_FORM);

		s.append("output:");
		s.append(getTableStartHTML());
		s.append(getTableRowHTML("parameter binding:",
				getURIHTML(((Resource) output.getProperty(OutputBinding.PROP_OWLS_BINDING_TO_PARAM)).getURI())));
		if (form == null)
			s.append(getTableRowHTML("--", "- unknown value, perhaps a unit conversion? -"));
		else
			s.append(getTableRowHTML("at the property path:", getPropPathHTML((PropertyPath) form, false)));
		s.append(getTableEndHTML());
	...
	protected String getServiceEffectHTML(Resource effect) {
		...
		if (ProcessEffect.TYPE_PROCESS_ADD_EFFECT.equals(type)) {
			s.append("Add effect:");
			s.append(getTableStartHTML());
			s.append(getTableRowHTML("add the value:",
					effect.getProperty(ProcessEffect.PROP_PROCESS_PROPERTY_VALUE).toString()));
			s.append(getTableRowHTML("to the property path:", getPropPathHTML(
					(PropertyPath) effect.getProperty(ProcessEffect.PROP_PROCESS_AFFECTED_PROPERTY), false)));
			s.append(getTableEndHTML());
		} else if (ProcessEffect.TYPE_PROCESS_CHANGE_EFFECT.equals(type)) {
			s.append("Change effect:");
			s.append(getTableStartHTML());
			s.append(getTableRowHTML("change the value to:",
					effect.getProperty(ProcessEffect.PROP_PROCESS_PROPERTY_VALUE).toString()));
			s.append(getTableRowHTML("at property path:", getPropPathHTML(
					(PropertyPath) effect.getProperty(ProcessEffect.PROP_PROCESS_AFFECTED_PROPERTY), false)));
			s.append(getTableEndHTML());
	...
	private HashSet<String> getNamedSuperClasses(Resource r) {
		...
		for (int i = 0; i < types.length; i++) {
			...
			OntClassInfo info = OntologyManagement.getInstance().getOntClassInfo(type);
			if (info != null) {
	 ...
	 * request as HTML.
	 */
	private void getServiceCommonHTML(StringBuilder s, Resource[] effects, Resource[] outputs) {
		int i;
	...
	protected void getServiceProfileHTML(StringBuilder s, ServiceProfile prof) {
		...
		if (!it.hasNext()) {
			...
			while (it.hasNext()) {
				...
				s.append("input:");
				s.append(getTableStartHTML());
				s.append(getTableRowHTML("URI:", getURIHTML(URI.get(in.getURI(), false))));
				String type = in.getParameterType();
				s.append(getTableRowHTML("type:", getURIHTML(URI.get(type, false))));
	...
	protected void getServiceRequestHTML(StringBuilder s, ServiceRequest req, String serviceURI) {
		...
		Object o = req.getRequestedService().getProperty(Service.PROP_INSTANCE_LEVEL_RESTRICTIONS);
		if (o instanceof List<?>) {
	 ...
	 * service request.
	 */
	protected void getAllServiceRequestHTML(StringBuilder s, ServiceRequest req, String serviceURI,
