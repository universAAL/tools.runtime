...
public class OntologyPane extends HTMLVisibilityPane {
	...
	private void createOverviewHTML(StringBuilder s) {
		...
		s.append(getVTableRowWithTitleHTML("Label", ont.getInfo().getResourceLabel()));
		s.append(getVTableRowWithTitleHTML("Comment", ont.getInfo().getResourceComment()));
		s.append(getVTableRowWithTitleHTML("RDF Classes", "" + ont.getRDFClassInfo().length));
		s.append(getVTableRowWithTitleHTML("Ontology Classes", "" + ont.getOntClassInfo().length));
		...
		for (OntClassInfo ci : info) {
			...
			String superClasses[] = ci.getNamedSuperClasses(false, true);
			{
				...
				for (String sc : superClasses) {
					s.append(getTableRowHTML(getURIHTML(c), getURIHTML(propURI), getURIHTML(sc)));
					c = "";
					propURI = "";
				}
			}
			...
			if (propURIs.length == 0 && superClasses.length == 0) {
				...
				for (String propURI : propURIs) {
					...
					if (res != null) {
						Object con = res.getConstraint(MergedRestriction.allValuesFromID);
					...
					s.append(getTableRowHTML(getURIHTML(c), getURIHTML(propURI), getURIHTML(o)));
					c = "";
				}
			}
		}
