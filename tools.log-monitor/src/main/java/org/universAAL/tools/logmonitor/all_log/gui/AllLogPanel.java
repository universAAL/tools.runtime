...
public class AllLogPanel extends JPanel implements ListSelectionListener {
	...
	public AllLogPanel(LogMonitor logMonitor) {
		...
		// Split pane
		JSplitPane split = new JSplitPane(JSplitPane.VERTICAL_SPLIT, scrollpaneTable, scrollpane);
	...
	public void addMessage(final LogEntry le) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				...
				model.addRow(new Object[] { Utils.getDateString(le.date), Utils.getLevel(le.logLevel), le.module,
						le.pkg, le.cls, le.method, Utils.buildMessage(le.msgPart) });
			}
		});
	}
