...
import org.universAAL.tools.ucc.configuration.configdefinitionregistry.interfaces.ConfigurationDefinitionRegistryChanged;
	...
	public WhichBundleShouldBeConfiguredWindow(String usecase) {
		...
		configure.addListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				// Start the configuration overview window with the selected
				// bundle.
				Configuration config = (Configuration) bundles.getValue();
				if (config != null) {
					ConfigurationOverviewWindow configWindow = (ConfigurationOverviewWindow) WhichBundleShouldBeConfiguredWindow.this
							.getApplication().getWindow(ConfigurationOverviewWindow.NAME);
