...
public class ConfigurationOverviewWindow extends Window
		implements ModelRegistryChangedListener, StorageChangedListener {
	...
	public ConfigurationOverviewWindow(/* String flatId, */ Configuration config) {
		...
		// �nderung
		// ServiceReference<DataAccess>ref =
		// context.getServiceReference(DataAccess.class);
	...
	private void initializeView() {
		...
		configurationTitle = new Label(
				"Configuration ID: " + controller.getConfigurator().getConfigDefinition().getId());
		bundleName = new Label(
				"Usecase bundle name: " + controller.getConfigurator().getConfigDefinition().getBundlename());
		...
		edit.addListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				// Set the selected configuration instance as configuration
	...
	private void fillSelect() {
		...
		for (ConfigurationInstance instance : storage
				.getAllInstancesForBundle(controller.getConfigurator().getConfigDefinition().getBundlename())) {
	...
	public void addMapConfigItemToPanel(Panel p, MapConfigurationOption mapOption) {
		...
		// multi = new MultiselectionList(controller, mapOption,
				...
				while (i.hasNext()) {
					...
					if (component instanceof ConfigurationPanel) {
						if (!((ConfigurationPanel) component).isValid()) {
							configurationWindow.showNotification(
									"Couldn't save because of validation or required errors!",
	...
	private void createViewFromModel() {
		...
		// }
		//
		// MapConfigurationOption map = new MapConfigurationOption(item, cat,
		// controller.getModelRegistry());
		// addMapConfigItemToPanel(cp, map);
		// for(Iterator<Component> iter = cp.getComponentIterator();
		// iter.hasNext();) {
	...
	// deviceAdress = val.getValue().substring(0,
	// val.getValue().indexOf("/")).trim();
	// ads.add(deviceAdress);
	// sensor = val.getValue().substring(val.getValue().indexOf("/")+ 1,
	// val.getValue().lastIndexOf("/")).trim();
	// room =
	// val.getValue().substring(val.getValue().lastIndexOf("/")+1).trim();
	// locs.add(room);
	// }
	//
	//
	...
	// }
	// int curr = 0;
	// for(Iterator<Component> it = configurationWindow.getComponentIterator();
	// it.hasNext(); curr++) {
