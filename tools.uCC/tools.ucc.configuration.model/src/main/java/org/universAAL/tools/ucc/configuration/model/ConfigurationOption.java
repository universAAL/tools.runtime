...
import org.universAAL.tools.ucc.configuration.model.configurationdefinition.ConfigItem;
import org.universAAL.tools.ucc.configuration.model.configurationdefinition.Dependency;
import org.universAAL.tools.ucc.configuration.model.configurationdefinition.Item;
import org.universAAL.tools.ucc.configuration.model.configurationdefinition.OnConfigurationModelChangedListener;
import org.universAAL.tools.ucc.configuration.model.configurationdefinition.Validator;
import org.universAAL.tools.ucc.configuration.model.configurationdefinition.Validators;
...
import org.universAAL.tools.ucc.configuration.model.servicetracker.ListenerServiceTracker;
...
public abstract class ConfigurationOption implements Comparable<ConfigurationOption> {
	...
	/**
	 * Loads the Validators specified by the configuration definition. This in
	 * done by reflection.
	 */
			...
			for (Validator validator : tmpValidators.getValidator()) {
				...
				if (className != null) {
					try {
						...
						// Class c =
						// ConfigurationValidator.class.getClassLoader().getParent().loadClass(className);
						for (Class theInterface : c.getInterfaces()) {
							if (theInterface.getName().equals(ConfigurationValidator.class.getName())) {
								try {
									...
									valInstance.setAttributes(validator.getAttribute()
											.toArray(new String[validator.getAttribute().size()]));
						...
						if (!"".equals(className)) {
							LogUtils.logInfo(Activator.getContext(), this.getClass(), "setValidators", new Object[] {
									"Class not found. Register service tacker for ValidatorCclass: " + className },
									null);
							...
							ValidationServiceTracker serviceTracker = new ValidationServiceTracker(
									...
									, className, this,
									validator.getAttribute().toArray(new String[validator.getAttribute().size()]));
							serviceTracker.open();
						}
					}
				}
	...
	/**
	 * Loads the OnConfigurationModelChangedListener specified by the
	 * configuration definition. This in done by reflection.
	 */
				...
				try {
					...
					for (Class theInterface : c.getInterfaces()) {
						if (theInterface.getName().equals(OnConfigurationChangedListener.class.getName())) {
							try {
								OnConfigurationChangedListener modListener = (OnConfigurationChangedListener) c
										.newInstance();
								addListener(modListener);
							} catch (InstantiationException e) {
								LogUtils.logError(Activator.getContext(), this.getClass(),
										"setOnChonfigurationModelChangedListener", new Object[] { e.toString() }, null);
					...
					if (!"".equals(className)) {
						LogUtils.logError(Activator.getContext(), this.getClass(),
								"setOnChonfigurationModelChangedListener",
								new Object[] { "Class not found. Register service tracker for class:" + className },
								null);

						ListenerServiceTracker serviceTracker = new ListenerServiceTracker(Activator.getBundleContext()
						// FrameworkUtil.getBundle(getClass()).getBundleContext()
								, className, this);
						serviceTracker.open();
					}
			...
			for (Value value : values) {
				if (!validator.isValid(configRegestry, value)) {
					LogUtils.logInfo(Activator.getContext(), this.getClass(), "allValidatorValid",
							new Object[] { "Validator failed: " + validator.getClass().getName() + " with input: "
									+ value.getValue() },
							null);
	...
	protected void updateExternalListeners() {
		for (OnConfigurationChangedListener listener : new LinkedList<OnConfigurationChangedListener>(
				externalListeners)) {
			try {
				listener.configurationChanged(configRegestry, this);
			} catch (Exception e) {
	 ...
	 * they are valid then this configuration option will be activated. If not
	 * it will be disabled.
	 */
