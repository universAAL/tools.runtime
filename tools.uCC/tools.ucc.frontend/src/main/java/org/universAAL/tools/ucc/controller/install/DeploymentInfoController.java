...
import org.universAAL.tools.ucc.windows.DeployConfigView;
import org.universAAL.tools.ucc.windows.DeployStrategyView;
...
import org.universAAL.tools.ucc.windows.SelectUserWindow;
import org.universAAL.tools.ucc.windows.SuccessWindow;
...
public class DeploymentInfoController implements Button.ClickListener, ValueChangeListener {
	...
	public DeploymentInfoController(UccUI app, AALService aal, DeploymentInformationView win, UAPP uAPP) {
		...
		for (UAPP ua : aal.getUaapList()) {
			for (Map.Entry<String, UAPPPart> applicationPart : ua.getParts().entrySet()) {
				...
				List<PeerCard> validpeers = getValidPeers(applicationPart.getValue().getReqAtoms(),
		...
		if (event.getButton() == win.getOk()) {
			...
			for (Map.Entry<String, UAPPPart> uapp : installingApp.getParts().entrySet()) {
				...
				if (uapp.getValue().getPart().getPartId().equals(selected.substring(0, selected.indexOf(":")))) {
					...
					else if (dsvMap.get(selected.substring(0, selected.indexOf(":"))).getOptions().getValue().toString()
	...
	private Map<Part, List<PeerCard>> buildDefaultInstallationLayout(UAPPPart uapp) {
		...
		if (validpeers.size() == 0 || validpeers == null) {
			app.getMainWindow().showNotification(bundle.getString("peer.available.not"),
			...
			System.out.println("[DeploymentInfoController] add peer " + validpeers.get(0).getPeerID() + " to "
	...
	private Map<Part, List<PeerCard>> buildUserInstallationLayout(UAPPPart uapp) {
		...
		if (dcvMap.get(selected).getSelect().getValue() != null
				&& !(dcvMap.get(selected).getSelect().getValue().toString().equals(""))) {
			Collection<String> selPeer = (Collection<String>) dcvMap.get(selected).getSelect().getValue();
			System.out.println("[DeploymentInfoController.buildUserLayout] user has selected "
					+ selPeer.toArray().length + " peers");
			for (int i = 0; i < selPeer.toArray().length; i++) {
				String value = dcvMap.get(selected).getPeerNodes().get(selPeer.toArray()[i]);
				...
				peerList.add(peer);
				System.out.println("[DeploymentInfoController.buildUserLayout] add one peer " + peer.getPeerID()
						+ " to part " + uapp.getPart().getPartId());
	...
	public static List<PeerCard> getValidPeers(List<UAPPReqAtom> reqs, String PartId) {
		...
		for (int i = 0; i < reqs.size(); i++) {
			...
			System.err.println("Name: " + reqs.get(i).getName() + "VALUE: " + reqs.get(i).getValue() + "Criteria: "
				...
				for (String sr : reqs.get(i).getValue()) {
					...
					if (reqcriteria != null) {
						if (reqcriteria.equals("greater-equal")) {
							System.err.println(
									"[[DeploymentInfoController]] in greater-equal criteria ReqValue: " + reqvalue);
	...
	private void showInstallationResultToUser(InstallationResultsDetails res) {
		...
		if (res.getGlobalResult().toString().equals(InstallationResults.SUCCESS.name())) {
			...
			for (Configuration configurator : reg.getAllConfigDefinitions()) {
				// System.err.println(uapp.getValue().getBundleId()+" +
				// "+configurator.getBundlename());

				// TODO:Has to be tested
				for (Map.Entry<String, UAPPPart> applicationPart : installingApp.getParts().entrySet()) {
					if (configurator.getBundlename() != null
							&& configurator.getBundlename().equals(applicationPart.getValue().getBundleId())) {
						conf = configurator;
					}
				}
			}
			...
			if (conf != null) {
				...
				ncw = new NoConfigurationWindow(bundle.getString("installed.note"));
